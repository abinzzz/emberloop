...
```sh
emberloop status --json      # Read-only quota/reset snapshot
emberloop run --dry-run      # Confirm eligibility, without sending a turn
emberloop run                # Check once, send only if eligible
emberloop watch              # Foreground monitor
emberloop watch --poll 60    # Check at most every 60 seconds
emberloop watch --transport cli  # Explicit native CLI alternative
```
...
- Homebrew service logs are in `$(brew --prefix)/var/log/emberloop.log` and `emberloop.error.log`. Foreground commands emit JSON events.
